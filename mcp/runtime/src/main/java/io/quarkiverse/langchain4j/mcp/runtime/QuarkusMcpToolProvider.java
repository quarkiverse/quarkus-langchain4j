...
public class QuarkusMcpToolProvider implements ToolProvider {
    ...
    private Collection<McpClient> getMcpClients(ToolProviderRequest request) {
        List<String> mcpServerNames = request instanceof QuarkusToolProviderRequest quarkusRequest
                ? quarkusRequest.getMcpServerNames()
                : null;
        if (mcpServerNames == null || mcpServerNames.isEmpty()) {
            return mcpClients.values();
        }
        return mcpServerNames.stream()
                .map(mcpServerName -> {
                    McpClient mcpClient = mcpClients.get(mcpServerName);
                    if (mcpClient == null) {
                        log.warn("Failed to find MCP client for server {}", mcpServerName);
                    }
                    return mcpClient;
                })
