...
import io.quarkiverse.langchain4j.mcp.auth.McpAuthProvider;
...
public class QuarkusHttpMcpTransport implements McpTransport {
    ...
    private volatile McpAuthProvider mcpAuthProvider;
    ...
    public QuarkusHttpMcpTransport(QuarkusHttpMcpTransport.Builder builder) {
        ...
        this.mcpAuthProvider = McpAuthProvider.resolve(builder.mcpClientName).orElse(null);
        if (mcpAuthProvider != null) {
            clientBuilder.register(new McpClientAuthFilter(mcpAuthProvider));
        }
    ...
    public void start(McpOperationHandler messageHandler) {
        ...
        QuarkusRestClientBuilder builder = QuarkusRestClientBuilder.newBuilder()
                ...
                .register(new JacksonBasicMessageBodyReader(QuarkusJsonCodecFactory.ObjectMapperHolder.MAPPER));
        if (mcpAuthProvider != null) {
            builder.register(new McpClientAuthFilter(mcpAuthProvider));
        }
        if (logRequests || logResponses) {
