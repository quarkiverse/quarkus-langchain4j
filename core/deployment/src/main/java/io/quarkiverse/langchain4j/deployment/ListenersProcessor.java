...
import io.quarkiverse.langchain4j.runtime.listeners.NoopChatModelSpanContributor;
...
public class ListenersProcessor {
    ...
    public void spanListeners(Capabilities capabilities,
            ...
            BuildProducer<AdditionalBeanBuildItem> additionalBeanProducer, CombinedIndexBuildItem combinedIndexBuildItem) {
        ...
        if (addOpenTelemetrySpan) {
            ...
            var noopChatModelSpanContributor = 1;
            var contributors = combinedIndexBuildItem.getIndex().getAllKnownImplementors(ChatModelSpanContributor.class);
            if (contributors.size() == noopChatModelSpanContributor) {
                additionalBeanProducer.produce(
                        AdditionalBeanBuildItem.builder().addBeanClass(NoopChatModelSpanContributor.class).setUnremovable()
                                .build());
            }
        }
