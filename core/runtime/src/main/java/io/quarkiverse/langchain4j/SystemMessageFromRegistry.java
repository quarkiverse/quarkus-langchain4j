...
/**
 * Loads the system message template of an AI service method (or of every method of an AI service interface) from a
 * prompt template registry (such as Apicurio Registry) at runtime instead of declaring it inline with
 * {@link dev.langchain4j.service.SystemMessage}.
 * <p>
 ...
 * <p>
 * {@code @SystemMessageFromRegistry} cannot be combined with {@link dev.langchain4j.service.SystemMessage} on the same
 * element. A method-level annotation takes precedence over a class-level one.
 */
@Retention(RUNTIME)
@Target({ METHOD, TYPE })
public @interface SystemMessageFromRegistry {
