...
public abstract class AbstractMockHttpMcpServer {
        ...
        if (message.get("method") != null) {
            ...
            if (method.equals("initialize")) {
                ...
                if (shouldRespondToPing) {
                    ...
                    sink.send(sse.newEventBuilder().name("message").data(result).build());
                } else {
    ...
    private void executeLoggingOperation(JsonNode message, String operationId) {
        ...
        sink.send(sse.newEventBuilder().name("message").data(log).build());
    ...
    private void listTools(String operationId) {
        ...
        sink.send(sse.newEventBuilder().name("message").data(response).build());
    }
    ...
    private void initialize(String operationId) {
        ...
        sink.send(sse.newEventBuilder().name("message").data(initializeResponse).build());
    }
    ...
    private void executeAddOperation(JsonNode message, String operationId) {
        ...
        ObjectNode result = buildToolResult(operationId,
                "The sum of " + a + " and " + b + " is " + additionResult + ".");
        sink.send(sse.newEventBuilder().name("message").data(result).build());
    }
