...
public class MongoDBEmbeddingStore implements EmbeddingStore<TextSegment> {
    ...
    public MongoDBEmbeddingStore(MongoClient mongoClient, String database, String collection, String indexName, String vectorFieldName, String textFieldName, String metadataFieldName) {
    ...
    public EmbeddingSearchResult<TextSegment> search(EmbeddingSearchRequest embeddingSearchRequest) {
        ...
        var itteratorResult = collection.aggregate(pipeline).
                iterator();
        ...
        if (availableResults > 0) {
            ...
            itteratorResult.forEachRemaining(document -> {
                var score = document.getDouble("vectorSearchScore");
                var id = document.getString("_id");
                var text = document.getString(textFieldName);
                result.add(new EmbeddingMatch<>(score, id, , ))
            });
        }else result = List.of();
