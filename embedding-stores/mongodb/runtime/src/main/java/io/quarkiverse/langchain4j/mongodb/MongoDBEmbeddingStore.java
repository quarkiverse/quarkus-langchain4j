...
public class MongoDBEmbeddingStore implements EmbeddingStore<TextSegment> {
    ...
    public EmbeddingSearchResult<TextSegment> search(EmbeddingSearchRequest request) {
        ...
        var queryResult = collection.aggregate(pipeline).into(new ArrayList<>());

        var result = queryResult.stream()
                .map(this::getTextSegmentEmbeddingMatch).toList();

        return new EmbeddingSearchResult<>(result);
    }

    private @NonNull EmbeddingMatch<TextSegment> getTextSegmentEmbeddingMatch(Document document) {
        ...
        return new EmbeddingMatch<>(score, id, Embedding.from(vector), textSegment);
    }
