...
public class AgenticDevUiJsonRpcTest extends DevUIJsonRPCTest {
    ...
    void monitoringPopulatesTopologyAndExecutions() throws Exception {
        ...
        JsonNode topology = executeJsonRPCMethod("getTopologyGraph", Map.of("index", 0));
        Assertions.assertThat(topology.get("nodes")).isNotEmpty();
        Assertions.assertThat(topology.get("links")).isNotEmpty();
    ...
    /**
     * The layout carries the workflow semantics: a sequence chains its sub-agents top to bottom, so
     * the root sits above them and each one flows into the next.
     */
    @Test
    void sequenceTopologyIsLaidOutAsAChain() throws Exception {
        JsonNode topology = executeJsonRPCMethod("getTopologyGraph", Map.of("index", 0));

        JsonNode root = nodeNamed(topology, "SequenceWrapper");
        Assertions.assertThat(root.get("topology").asText()).isEqualTo("SEQUENCE");

        for (JsonNode node : topology.get("nodes")) {
            if (node != root) {
                Assertions.assertThat(node.get("y").asDouble()).isGreaterThan(root.get("y").asDouble());
            }
        }

        Assertions.assertThat(topology.get("links")).anySatisfy(
                link -> Assertions.assertThat(link.get("kind").asText()).isEqualTo("flow"));
    }

    private static JsonNode nodeNamed(JsonNode topology, String name) {
        for (JsonNode node : topology.get("nodes")) {
