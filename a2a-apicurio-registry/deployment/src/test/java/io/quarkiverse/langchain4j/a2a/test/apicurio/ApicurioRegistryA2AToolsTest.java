...
public class ApicurioRegistryA2AToolsTest {
    ...
    @Test
    @Order(2)
    void registrySearchFindsAgentCards() {
        RegistryClientOptions options = RegistryClientOptions.create(registryUrl()).httpAdapter(HttpAdapterType.JDK);
        RegistryClient client = RegistryClientFactory.create(options);
        ...
        assertThat(results.getArtifacts()).isNotEmpty();
        var names = results.getArtifacts().stream().map(a -> a.getName()).toList();
        assertThat(names).contains("Translation Agent", "Sentiment Analysis Agent");
    }

    @Test
    @Order(3)
    void allAgentsReturnsEmptyWhenA2AServersUnreachable() {
        var agents = agentsRegistry.allAgents();
