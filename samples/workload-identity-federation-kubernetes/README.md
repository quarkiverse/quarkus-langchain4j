...
No API keys are used — the workload proves its identity through a Kubernetes-issued JWT token, which is exchanged for a short-lived Anthropic access token.
...
- `jq` (optional, can be used to decode the Kubernetes service account token claims)
...
## 6. Verify the token issuer URL and subject
...
oc exec deployment/workload-identity-federation-kubernetes -n workload-identity-k8s -c app \
    -- cat /var/run/secrets/tokens/kubernetes-service-token | cut -d. -f2 | base64 -d 2>/dev/null | jq
```
