...
public class McpAuthorizationResource {
    ...
    public Response addConnectionPost(@QueryParam("name") String name,
                                  @QueryParam("transportType") String transportType,
                                  @QueryParam("command") String command,
                                  @QueryParam("url") String url) {
        // Not expected to be called - initial request uses POST before the challenge
        // but on the redirect back from the authorization server it uses GET 
        return addConnection(name, transportType, command, url);
    }
