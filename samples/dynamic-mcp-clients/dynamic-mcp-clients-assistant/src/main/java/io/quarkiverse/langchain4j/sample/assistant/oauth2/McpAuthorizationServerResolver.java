...
public class McpAuthorizationServerResolver implements TenantConfigResolver {
    ...
    public Uni<OidcTenantConfig> resolve(RoutingContext routingContext,
        ...
        if (!routingContext.request().path().endsWith("/mcp-authorization")) {
            return null;
        }
        
        String resolvedTenantId = routingContext.get(OidcUtils.TENANT_ID_ATTRIBUTE);
        if (resolvedTenantId != null) { 
            Log.infof("OidcTenantConfig %s tenant configuration is resolved", resolvedTenantId);
            return null;
        }
        
        
        if (!OidcUtils.isFormUrlEncodedRequest(routingContext)) {
            Log.warnf("OidcTenantConfig resolver requires a form request");
            return null;
        }    
        
        return OidcUtils.getFormUrlEncodedData(routingContext).onItem()
            .transformToUni(new Function<MultiMap, Uni<? extends OidcTenantConfig>>() {
                @Override
                public Uni<? extends OidcTenantConfig> apply(MultiMap formParams) {
                    return resolveOidcTenantConfig(routingContext, formParams);
                }
            });
    }

    private Uni<OidcTenantConfig> resolveOidcTenantConfig(RoutingContext routingContext, final MultiMap formParams) {
        ...
        return mcpServerAuthorizationServerUni.onItem().transform(new Function<McpServerAuthorizatonServer, OidcTenantConfig>() {
            ...
            public OidcTenantConfig apply(McpServerAuthorizatonServer server) {
                ...
                if (clientIdAndSecret.clientSecret() != null && !clientIdAndSecret.clientSecret().isEmpty()) {
                    builder.credentials(clientIdAndSecret.clientSecret());
                }
                ...
                final List<String> scopes = getScopes(formParams);
                if (!scopes.isEmpty()) {
    ...
    protected ClientIdAndSecret getClientIdAndSecret(RoutingContext routingContext, MultiMap formParams) {
        ClientIdAndSecret credentials = checkFormParameters(formParams);
        if (credentials == null) {
            //TODO: register client using Client Metadata or Dynamic Client Registration
        }
        return credentials;        
    }
    ...
    static List<String> getScopes(MultiMap formParams) {
        String scopesString = formParams.get("scopes");
        if (scopesString == null || scopesString.trim().isEmpty()) {
            return List.of();
        }
        return List.of(URLDecoder.decode(scopesString.trim(), StandardCharsets.UTF_8).split("\\s+"));
    }
    ...
    static record ClientIdAndSecret(String clientId, String clientSecret) {
