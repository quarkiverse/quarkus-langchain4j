...
This Quarkus MCP server can be optionally used to support an assistant that you can launch in the `dynamic-mcp-clients-assistant` folder.
...
When you will attempt to use an assistant to import `http://localhost:8081/mcp`, the assistant will request an authentication and ask to enter an OAuth2 Client ID. Enter an OAuth2 `alpha-client` Client ID, and login as a user `alice` with a password `alice`.
