...
The registry stores MCP tool definitions as versioned `MCP_TOOL` artifacts, validated against the official
https://modelcontextprotocol.io/specification/2025-11-25/server/tools[MCP specification]. Tool definitions follow the spec
(`name`, `inputSchema`, optional `description`, `title`, `annotations`, `outputSchema`), while connection metadata
(server URL, transport type) is stored as **artifact labels** -- keeping the artifact content spec-compliant.
The Quarkus integration provides `@Tool`-annotated methods that allow the LLM itself to search the registry and connect to
discovered servers -- no redeployment or config changes needed when new MCP servers become available.

IMPORTANT: This feature is experimental and may change in future releases.

NOTE: MCP clients added through the registry integration are currently global — because the tool provider is application-scoped, the tools from dynamically connected servers will be visible to all users. Per-session scoping is planned for a future release.

=== Dependencies

Add the Apicurio Registry MCP extension alongside the MCP extension:
...
<dependency>
    <groupId>io.quarkiverse.langchain4j</groupId>
    <artifactId>quarkus-langchain4j-mcp-apicurio-registry</artifactId>
</dependency>
----
...
When the `quarkus-langchain4j-mcp-apicurio-registry` extension is on the classpath and the configuration above is present, Quarkus automatically registers
...
=== Registering MCP Tool Definitions

MCP tool definitions are stored in Apicurio Registry as `MCP_TOOL` artifacts. The artifact **content** follows the
...
Connection metadata is stored as **artifact labels** (key-value metadata on the artifact):

- `mcp-server-url` -- the URL of the MCP server hosting this tool (e.g., `http://filesystem-server:8080/mcp`)
- `mcp-transport-type` -- the transport type (e.g., `streamable-http`)

The registry validates tool definitions against the MCP spec using its content validation rules. You can register
these artifacts through the Apicurio Registry UI, REST API, or CLI.
...
MCP defines the log payload as any JSON-serializable value, so `dataAsObject()` returns whatever the
server sent - commonly a `String`, sometimes a `Map`. Use `dataAsMap()` if you read named fields out
of it, which returns `null` when the payload is not an object, or `dataAsJson()` to get it back as
JSON text.
