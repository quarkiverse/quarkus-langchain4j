...
|Maximum number of LLM request/response round trips allowed per request.
|10, but see the note
|If not set, the value of the `quarkus.langchain4j.ai-service.max-tool-calling-round-trips` property will be used. The default value of that property is 10.
