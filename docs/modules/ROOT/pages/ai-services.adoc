...
|Maximum number of sequential tool invocations allowed per request.
|10, but see the note
|If not set, the value of the `quarkus.langchain4j.ai-service.max-tool-executions` property will be used. The default value of that property is 10.
...
Implement `SystemMessageProvider` when the message only depends on the memory ID, or `SystemMessageProviderWithContext` when it needs to inspect the model in use.

`InvocationContext` exposes the model at two levels of granularity: `modelProvider()` gives the provider (e.g. `ModelProvider.OPEN_AI`), while `defaultRequestParameters().modelName()` gives the exact model name in use (e.g. `gpt-4o` vs `gpt-4o-mini`), which lets the system message vary per model, not just per provider:
...
public class ModelAwareSystemMessageProvider implements SystemMessageProviderWithContext {
    ...
    public Optional<String> getSystemMessage(InvocationContext context) {
        ...
        if ("gpt-4o-mini".equals(modelName)) {
            return Optional.of("You are a helpful assistant. Be very concise, this model has a small context window.");
        }
...
Register it on the AI service with `systemMessageProviderSupplier`:
...
To load the system message from Apicurio Registry at runtime instead, use `@SystemMessageFromRegistry`
(see xref:prompt-and-template.adoc#prompt-registry[Loading Prompts from Apicurio Registry]).
...
To load the user message template from Apicurio Registry at runtime instead, use `@UserMessageFromRegistry`
(see xref:prompt-and-template.adoc#prompt-registry[Loading Prompts from Apicurio Registry]).
...
== Per-request Chat Parameters

The model configuration described above applies to all requests made by the AI Service.
If you need to override chat parameters (such as `temperature` or `topP`) on a per-request basis, you can add a `ChatRequestParameters` parameter to any AI Service method.

[source,java]
----
@RegisterAiService
public interface MyAiService {
    String chat(@UserMessage String message, ChatRequestParameters params);
}
----

Then, at call time, build the parameters you want to override:
...
String response = myAiService.chat("Hello!", params);
----
...
Handle tool execution errors with custom error handlers defined as static methods in your AI Service interface.

==== @HandleToolArgumentError

Handles invalid arguments provided by the LLM to tool calls.
...
public interface MyAiService {
    ...
    @HandleToolArgumentError
    static String handleArgumentError(Throwable error) {
...
Handles runtime exceptions during tool execution.
...
public interface MyAiService {
    ...
    @HandleToolExecutionError
    static ToolErrorHandlerResult handleExecutionError(Throwable error, ToolErrorContext context) {
        return ToolErrorHandlerResult.text("Tool '" + context.toolName() + "' failed: " + error.getMessage());
    }
}
----
...
- Must be `static`
- Parameters (optional): `Throwable` and/or `ToolErrorContext`
- Return type: `String` or `ToolErrorHandlerResult`
