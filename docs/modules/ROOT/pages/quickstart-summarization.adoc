...
====
See xref:llms.adoc[Models Serving] for other available LLM integrations.
====
...
[.lead]
- Explore xref:guide-structured-output.adoc[structured outputs] to return more complex objects
[.lead]
- Try adding support for xref:guide-passing-images.adoc[text+image inputs]
[.lead]
- Add xref:observability.adoc[observability] to monitor prompt usage and performance
