...
* *Anonymization and Pseudonymization*: Where possible, replace real data with placeholders or anonymized versions. Techniques like data masking can help protect sensitive information while preserving the meaning for the LLM.
...
* When using providers like Gemini or Azure OpenAI, prefer short-lived access tokens obtained during login instead of long-lived API keys. See https://docs.quarkiverse.io/quarkus-langchain4j/dev/llms.html#_access_to_llms[Access to LLMs].
