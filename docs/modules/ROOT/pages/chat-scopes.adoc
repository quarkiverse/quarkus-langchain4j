...
Quarkus Chat Scopes is an extension built on top of link:https://docs.quarkiverse.io/quarkus-langchain4j/dev/index.html[Quarkus Langchain4j] to help you manage local and remote conversations
...
* Another new CDI scope (`@InvocationScoped`).  A scope that starts and ends with a bean method call.  Great for one-off AiService method calls.
* A new event-based chat invocation framework for executing on beans and AiServices (`@ChatRoute`).
* A local and remote client API (java and javascript) that can be used to chat with enabled beans and AiServices.  Great for developing Chat UIs.
* APIs for tighter control over Chat Memory

== New CDI conversational scope: `@ChatScoped`

Quarkus Chat Scopes introduces a new conversational scope that sits between `@SessionScoped` and `@RequestScoped`.  It is manually started and stopped.
...
This scope is great for one-off cases where you only want to hold state for the duration of a method call.  Think of it as a finer grain
...
state and chat memory for only the duration of an AiService method call that has multiple tool calls.
...
Chat scopes also comes with a chat invocation framework.  It provides a lot of structure for writing chat UI applications by allowing you to define chat endpoints and providing a local and remote client to synchronously and asynchronously invoke on those endpoints.  These endpoints are called chat routes and declared by applying the `@ChatRoute` annotation to any bean or AiService method.
...
the user message to the chat route endpoint and receives events asynchronously as responses to render.
...
Let's walk through an example.  Here's the chat route we are going to create a client for:
...
Return values from AiService methods automatically get turned into events that are sent back to the client.  A `String` return value
automatically gets converted into a `Message` event and the string is sent back within that event.  There's mappings for other return types, but we'll discuss that later.
...
some event handlers.  The helper method `messageHandler()` registers a handler that just consumes string "Message" events —
here we're registering `System.out.println`.  Same for error events.  The `SessionBuilder.connect()` method establishes a session and
specifies that the initial chat route will be `RagBot`.  Chat messages will be routed to `RagBot.chat()`.

So, as the client accepts command line input, it sends that string as a user message to the chat route service which determines that the current route is `RagBot`
...
public interface LocalChatRoutes {
    ...
    public interface SessionBuilder {
        ...
        /**
         * Start a session with the event bus using the initial route as the chat route
         *
         * @param initialRoute
         * @return
         */
        ...
        /**
         * Start a session with the event bus using the default route as the chat route
         *
         * @return
         */
        Session connect();
...
unless the application has a `@DefaultRoute` defined.  Only one default route per application is allowed.
...
Events are defined as a string type name and data.  Chat routes defines some convenience built-in event types:
...
You can define your own event types.  Just choose the type name and data format you want.
...
* If the return value is a `dev.langchain4j.service.Result`, the event type
...
event types are available (i.e. message, thinking, and error events).  Server code just needs to `@Inject` the `ChatRouteContext` and can send back messages to the client anytime during chat request processing.  For instance, maybe you want to send back a _thinking_ message back to the client while within LLM tool execution so that the user gets some feedback while waiting for the LLM to respond.  Here's an example:
...
* Calling `ChatScope.pop()` not only pops and reverts the current chat scope to the parent scope, but also reverts to the route associated with the parent chat scope
...
The URL for the web socket endpoint is `/_chat/routes`.  All communication with clients uses
...
Since the communication with the server uses JSON text messages, the `JsonEvent` interface is used instead of raw `java.lang.Object` instances for non-string event types.

[source,java]
----
    /**
     * Events are sent back as json to the client.  This interface is used
     * to get at the raw json data, or to deserialize it into a specific type.
     */
    interface JsonEvent {
        /**
...
Here's an example using custom event types with the websocket client:
...
If using websockets, secure using link:https://quarkus.io/guides/websockets-next-reference#websocket-next-security[Quarkus Websocket Next mechanisms].

Place security annotations like `@RolesAllowed` on your `@ChatRoute` methods as needed.

=== Route exception handling

If an exception is thrown from chat route code, the default behavior is that a `failure` event is sent
back to the client and the client will abort with a failure exception.

If you do not want an exception to cause a `failure` event, then the throw exception must be wrapped by
or extended from `HandledApplicationException`  Sometimes you can handle an error condition by sending back an event to the client yourself.  At the same time
you might be too far down the call stack.  In this case, use `HandledApplicationException`.time

==== Exception Handlers

You can set up exception handlers to catch exceptions thrown by your chat route methods by applying
the `@ChatRouteExceptionHandler` annotation on a method.  This method must be a public method
with 2 parameters.  The first parameter is the exception type you want to map to.  It can be a superclass
(i.e. `RuntimeException`).  The second parameter must be a `ChatRouteContext`.  The method can
be `static` or a member method.  If it is a member method, the class must be a CDI bean.

The `@ChatRouteExceptionHandler` has a `value()` attribute.  If left unset, then the
handler defined is a default handler for the entire application.  Otherwise, the values
specified pertain to a chat route.  When the current chat route is set to one of the values on the
annotation, that exception mapper will be triggered.

Here's an example of some default handlers:
[source,java]
----
...
@ApplicationScoped
public class MyDefaultHandlers {
    @ChatRouteExceptionHandler
    public void handleException(Exception ex, ChatRouteContext ctx) {
        ctx.response().error(ex.getMessage());
    }
}

public class NonBeanDefaultHandlers {
    @ChatRouteExceptionHandler
    public static void handleException(IllegalStateException ex, ChatRouteContext ctx) {
        ctx.response().error("Weird state of things");
    }
}
----

Notice that `MyDefaultHandlers.handleException` is within a bean.  It also handles any
exception type.  Notice that `NonBeanDefaultHandlers.handleException` is not a bean and has
a static method for a handler.  If an `IllegalStateException` is thrown, then `NonBeanDefaultHandlers.handleException`
will be called because it exactly matches the thrown exception type.
...
@ApplicationScoped
public class SpecificChatRouteHandler {
    @ChatRouteExceptionHandler("my-route-1", "my-route-2")
    public void handleException(SecurityException ex, ChatRouteContext ctx) {
        ctx.response().error(ex.getMessage());
    }
}
...
For the above example, if `SecurityException` is thrown, `SpecificChatRouteHandler.handleException`
will be called if and only if the current route is `my-route-1` or `my-route-2`.
...
Call `clearMemory()` anytime and it will immediately clear any chat memory for any chat scoped AiService accessed within the current chat scope.
...
within the current chat request.  Instead, you `scheduleWipe()`.  This will clear chat memory for the current chat scope when the
...
== Chat Scope CDI Lifecyle events

Chat scopes will synchronously fire off lifecycle events

|===
|Event Type|Description

|ChatScopeStarted|Beginning of a scope or a subscope (via a push)
|ChatScopeEnded|End of a scope (popped)
|ChatScopeActivated|When the scope is activated (not begun!).  Happens every request.
|ChatScopeDeactivated|When the scope is deactived (but not ended!).  Happens at end of every request.
|===

Each event class has access to the `ChatScope` in question.  From that object you can get the
chat id, current route, and parent scope.
...
The user message has data needed to book the flight: the user wants to go to Ireland in July.
...
When the `flight` chat route is done it should call `ChatScope.pop()` within one of its tool methods.  This will clean up any state and chat memory associated with
...
For our travel agent app that we started in the previous section, there's a bunch of nested conversations we need to have with the LLM to build up or create various things.
...
public class Flight {
    ...
    @Tool("specify the return date")
    public void setReturnDate(Date date) {
        ctx.response().thinking("Set return date to: " + date);
...
were used, but if you called request scoped AiServices within the same request, the chat memory of the previous request would be added to the second invocation.
