...
Streaming tool dispatch is batch-oriented: tools are dispatched after the model completes the
response that contains the tool requests. The cap is checked before dispatch, so exceeded batches
execute zero tools. A value of 0 disables the cap, but does not restore per-tool early dispatch.
