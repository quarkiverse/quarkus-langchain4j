...
Sign up for https://cloud.langfuse.com/[Langfuse Cloud] or https://langfuse.com/self-hosting[self-host Langfuse], then configure the connection in `application.properties`:
...
[TIP]
====
In dev and test mode, https://docs.quarkiverse.io/quarkus-langfuse/dev/#devservices[Langfuse DevServices] automatically starts a local Langfuse instance and configures the connection properties for you — no manual configuration needed.
====

[NOTE]
====
See the https://docs.quarkiverse.io/quarkus-langfuse/dev/all-config.html[Quarkus Langfuse documentation] for additional configuration options, such as changing the span filter mode or customizing timeouts.
====
...
A complete example can be found https://github.com/langfuse/langfuse-examples/tree/main/applications/quarkus-langchain4j-demo[here].
...
See the https://langfuse.com/docs/opentelemetry/get-started[Langfuse OpenTelemetry documentation] for more details on authentication and manual setup.
