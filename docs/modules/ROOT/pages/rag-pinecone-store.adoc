...
:provider-artifact: quarkus-langchain4j-pinecone
...
You can configure multiple named Pinecone stores, each with its own API key, environment, project, and index.
This is useful when your application needs to manage embeddings for different domains or tenants in separate Pinecone indexes.
...
[IMPORTANT]
====
The `quarkus.langchain4j.pinecone.<store-name>.index-name` property is a build-time property that must be set to enable the named store.
It also serves as the default index name for the store at runtime; if you need to override it at runtime, set the runtime `index-name` property as well.
====
