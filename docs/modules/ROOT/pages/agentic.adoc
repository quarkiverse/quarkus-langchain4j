...
* All supplier methods (`@ToolsSupplier`, etc.), except the `@ChatModelSupplier` one, must be `static` with correct return types and no parameters
* `@ChatModelSupplier` methods must be `static` and return `ChatModel` (parameters are allowed for dynamic model selection)
