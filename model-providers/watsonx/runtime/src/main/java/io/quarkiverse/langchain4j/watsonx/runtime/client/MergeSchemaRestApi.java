...
import com.ibm.watsonx.ai.textprocessing.schema.merge.MergeSchemaRequest;
...
@Path("")
public interface MergeSchemaRestApi {

    @POST
    @Path("/ml/v1/text/schemas/merge")
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    MergeSchemaResponse startRequest(
            @HeaderParam(REQUEST_ID_HEADER) String watsonxAISDKRequestId,
            @HeaderParam(TRANSACTION_ID_HEADER) String transactionId,
            @QueryParam("version") String version,
            MergeSchemaRequest request);

    @GET
    @Path("/ml/v1/text/schemas/merge/{request_id}")
    @Produces(MediaType.APPLICATION_JSON)
    MergeSchemaResponse fetchRequestDetails(
