...
import io.quarkiverse.langchain4j.ollama.OllamaMessagesUtils;
...
public class ExperimentalSequentialToolsDelegate implements ChatLanguageModel {
    ...
    public Response<AiMessage> generate(List<ChatMessage> messages,
        ...
        if (toolResponse.name.equals(DEFAULT_RESPONSE_TOOL.name())) {
            return Response.from(AiMessage.from(toolResponse.inputs.get("response").toString()),
                    new TokenUsage(response.promptEvalCount(), response.evalCount()), FinishReason.STOP);
        }
    ...
    private Message createSystemMessageWithTools(List<Message> messages, List<ToolSpecification> toolSpecifications) {
        ...
        String history = messages.stream().filter(sm -> sm.role() == Role.USER)
                .skip(1)
                .map(Message::content)
                .collect(Collectors.joining("\n"));
