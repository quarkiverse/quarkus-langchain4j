...
class ExperimentalMessagesUtils {
    ...
    final static Predicate<ChatMessage> isToolsResultMessage = chatMessage -> chatMessage instanceof ToolExecutionResultMessage;
    final static Predicate<ChatMessage> isToolsRequestMessage = chatMessage -> chatMessage instanceof AiMessage
            && ((AiMessage) chatMessage).hasToolExecutionRequests();
    final static Predicate<ChatMessage> isAiStatsMessage = chatMessage -> chatMessage instanceof AiStatsMessage;
    final static Predicate<UserMessage> hasImages = userMessage -> userMessage.contents().stream()
    ...
    static String toText(ToolExecutionRequest toolExecutionRequest) {
        return String.format("%s (%s)", toolExecutionRequest.name(), toolExecutionRequest.arguments().replace("\n", ""));
    }
